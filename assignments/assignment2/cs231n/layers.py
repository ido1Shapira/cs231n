...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        cache = x, mu, var, gamma, x_normalized, eps, axis
...
def batchnorm_backward(dout, cache):
    ...
    x, mu, var, gamma, x_normalized, eps, axis = cache
    N, D = x.shape
    ...
    dvar = np.sum(dx_normalized * (x - mu) * (-0.5) * (var + eps)**(-1.5), axis=0)
    dmean = np.sum(dx_normalized * (-1 / np.sqrt(var + eps)), axis=0) + dvar * np.sum(-2 * (x - mu), axis=0) / N
    
    dx = dx_normalized / np.sqrt(var + eps) + dvar * 2 * (x - mu) / N + dmean / N
...
def batchnorm_backward_alt(dout, cache):
    ...
    x, _, var, gamma, x_normalized, eps, axis = cache
    std = np.sqrt(var + eps)

    S = lambda x: x.sum(axis=0)
    
    dbeta = dout.reshape(x.shape, order='F').sum(axis=axis)
    dgamma = (dout * x_normalized).reshape(x.shape, order='F').sum(axis=axis)
    
    dx = dout * gamma / (len(dout) * std)
    dx = len(dout)*dx  - S(dx*x_normalized)*x_normalized - S(dx)
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    HO = 1 + int((HI + 2 * P - HF) / S)
    WO = 1 + int((WI + 2 * P - WF) / S)

    to_fields = lambda x: np.lib.stride_tricks.sliding_window_view(x, (WF, HF, C, N))

    w_row = w.reshape(F, -1)
    x_pad = np.pad(x, ((0,0), (0,0), (P, P), (P, P)), 'constant')
    x_col = to_fields(x_pad.T).T[...,::S,::S].reshape(N, C * HF * WF, -1)
...
def conv_backward_naive(dout, cache):
    ...
    N, _, Hpad, Wpad = x_pad.shape

    dout = np.insert(dout, [*range(1, HO)] * (S-1), 0, axis=2)
    dout = np.insert(dout, [*range(1, WO)] * (S-1), 0, axis=3)
    ...
    # same as: db = np.sum(dout, axis=(0, 2, 3))
...
def max_pool_forward_naive(x, pool_param):
    ...
    x_fields = to_fields(x.T).T[...,::S,::S].reshape(N, C, HP*WP, -1)
...
def max_pool_backward_naive(dout, cache):
    ...
    S = pool_param['stride'] # stride: up = down
    HP = pool_param['pool_height'] # pool height
    WP = pool_param['pool_width']  # pool width

    for i in range(HO):
        for j in range(WO):
            [ns, cs], h, w = np.indices((N, C)), i*S, j*S    # compact indexing
